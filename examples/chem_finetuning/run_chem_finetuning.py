...
class DataTrainingArguments:
    ...
    remove_characters: Optional[str] = field(
        default=None,
    )
...
def main():
    ...
    send_example_telemetry("run_finetuning", model_args, data_args)
    ...
    is_regression = data_args.dataset_name in ["jxie/esol", "jxie/freesolv", "jxie/lipop"]
    if is_regression:
        loss_fn = MSELoss()
    else:
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        if data_args.remove_characters is not None:
            for c in data_args.remove_characters:
                examples["text"] = [i.replace(c, "") for i in examples["text"]]
    ...
    eval_dataset = eval_dataset.cast_column("label", Value("float32"))
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            return {"rmse": metric.compute(references=p.label_ids, predictions=preds, squared=False)["mse"]}
        else:
            probs = torch.Tensor(p.predictions).sigmoid()
            return {"roc_auc": metric.compute(references=p.label_ids, prediction_scores=probs)["roc_auc"]}
