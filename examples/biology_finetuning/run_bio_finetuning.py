...
def main():
    ...
    def ss_loss(logits, labels):
        labels = [i[i != -1] for i in labels]
        labels = [l[:max_seq_length].to(logits.device) for l in labels]
        logits = [logits[i][:min(len(labels[i]), max_seq_length)] for i in range(len(logits))]
        #criterion = CrossEntropyLoss(reduction="none")
        criterion = CrossEntropyLoss()
        #print(torch.cat(logits).shape, torch.cat(labels).shape)
        return criterion(torch.cat(logits), torch.cat(labels))
        #losses = torch.cat([criterion(lo, la) for lo, la in zip(logits, labels)])
        #return losses.mean()
    
    is_regression = False
    if data_args.dataset_name in ["jxie/fluorescence", "jxie/stability"]:
        is_regression = True
        num_labels = 1
    else:
    ...
    def pad_label(label):
        ...
        return torch.cat([torch.LongTensor(label), -torch.ones((max_seq_length - len(label)))]).long()
