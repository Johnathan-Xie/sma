...
def create_vocab(dataset, key="text"):
    unique_values = list(set(x for data in tqdm(dataset) for x in data[key]))
    return {x: idx for idx, x in enumerate(unique_values)}

class SMATokenizer(PreTrainedTokenizer):
    """
