...
class Trainer:
    ...
    def get_optimizer_cls_and_kwargs(args: TrainingArguments) -> Tuple[Any, Any]:
        ...
        if args.optim == OptimizerNames.ADAFACTOR:
            ...
            optimizer_cls = AdamW
            optimizer_kwargs.update(adam_kwargs)
        elif args.optim == OptimizerNames.LAMB_TORCH:
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_step, auxiliary_logs = self.training_step(model, inputs)
                else:
                    tr_loss_step, auxiliary_logs = self.training_step(model, inputs)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval, auxiliary_logs=auxiliary_logs)
                else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval, auxiliary_logs={}):
        if self.control.should_log:
            ...
            logs.update({k: v.item() for k, v in auxiliary_logs.items()})
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        return loss.detach(), outputs["auxiliary_logs"] if outputs.get("auxiliary_logs") is not None else {}
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, auxiliary_logs = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            for k, v in auxiliary_values_host.items():
                auxiliary_logs[k] = nested_numpify(v)
                auxiliary_values_host[k] = auxiliary_logs[k] if auxiliary_values_host.get(k) is None else np.concatenate((auxiliary_values_host[k], auxiliary_logs[k]), axis=0)
            if loss is not None:
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                for k, v in auxiliary_values_host.items():
                    host_value = nested_numpify(v)
                    all_auxiliary_values[k] = host_value if all_auxiliary_values.get(k) is None else np.concatenate((all_auxiliary_values[k], host_value), axis=0)
                ...
                losses_host, preds_host, inputs_host, labels_host, auxiliary_values_host = None, None, None, None, {}
        ...
        for k, v in all_auxiliary_values.items():
            all_auxiliary_values[k] = v[:num_samples]
    ...
    def prediction_step(
        ...
        if prediction_loss_only:
            return (loss, None, None, outputs["auxiliary_logs"] if outputs.get("auxiliary_logs") is not None else {})

        logits = nested_detach(logits)
        if len(logits) == 1:
            logits = logits[0]

        return (loss, logits, labels, outputs["auxiliary_logs"] if outputs.get("auxiliary_logs") is not None else {})
    ...
    def prediction_loop(
        ...
        auxiliary_values_host: Dict[str, torch.Tensor] = {}
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if auxiliary_logs is not None:
                for k, v in auxiliary_logs.items():
                    if auxiliary_values_host.get(k) is None:
                        auxiliary_values_host[k] = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=batch_size)
                    values = v.repeat(batch_size)
                    auxiliary_values_host[k] = values if auxiliary_values_host.get(k) is None else torch.cat((auxiliary_values_host[k], losses), dim=0)
            if loss is not None:
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                for k, v in auxiliary_values_host.items():
                    auxiliary_values_gatherer[k].add_arrays(self._gather_and_numpify(v, k))
                if not prediction_loss_only:
                ...
                # Set back to None to begin a new accumulation
                losses_host, preds_host, labels_host, inputs_host, auxiliary_values_host = None, None, None, None, {}
        ...
        auxiliary_values_gatherer = {k: v.finalize() for k, v in auxiliary_values_gatherer.items()}
        ...
        for k, v in auxiliary_values_gatherer.items():
            metrics[k] = v.mean().item()
